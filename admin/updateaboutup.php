...
<body bgcolor='#FFC300'>
	<div class='container mt-3'>
		...
		$select_query = "select * from aboutus where id = $id";
		...
		<div style="font-size:20px;" align="center">
			<form 
			action='updateaboutup.php?id=<?php echo $row['id']?>&old_image=<?php echo $row['image'] ?>'
			method='post'
	...
	if(isset($_POST['updatebtn']))
	{
		...
		require '../dbcon.php';
		$update_query="update aboutus set title='$title', description='$description', image='$image' where id=$id ";
		if($con->query($update_query))
		{
			echo "<script>alert('data has been updated');</script>";
			echo "<script>window.location = updateaboutup.php?id='+$id + '&old_image=' + $image;</script>";
		}
		else
		{
